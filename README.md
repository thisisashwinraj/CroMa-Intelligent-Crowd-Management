...
The hardware cycle starts with the bus conductor initializing the trip in the handheld ticketing machine by entering the BusId. The machine is now configured to the route associated with the BusId and reflects the pre-set values for the three passenger data parameters that are: total number of available seats, the current location and total number of passengers in bus. The ticketing machine now shows options for printing the tickets, displaying trip details (used by the conductors) and showing the total fare collections during the trip.

When a new passenger boards the bus, the bus conductor issues them a ticket after collecting information about their point of origin, their destination and the number of co-passengers, if any. Apart from printing tickets, the proposed ticketing machine sends this data to the firebase real-time database wherein the values for the previously mentioned three parameters are updated. The location data are collected by the GPS module. For passenger using concession cards, a QR code scanner is used. This ensures the authenticity of the card holder, as well as the proper updation of the crowd data.
...
Users waiting for on-boarding a bus can open the CroMa application on their mobile device. They need to initialize the application by posting a query, enteringg their destination, onboarding point and the required bus type. The app then fetches the list of all nearby buses that passes through their current location, & destination from the firebase real time databse and displays it to the user. The user can select a particular bus to view more details about the bus including the available number of seats, crowd levels in the bus, timings for buses with similar route, and the expected fare, etc.

The application uses Maps SDK to display the dynamic in-app map interface. The Directions API displays the directions of buses, featuring real-time traffic information. The Distance Matrix API isused to calculate the travel time and distances between the origins and destinations. The  Geocoding API from Maps is used to convert coordinates into addresses and addresses into coordinates. Finally the Places SDK integrates place details, search, & autocomplete into the CroMa app.
